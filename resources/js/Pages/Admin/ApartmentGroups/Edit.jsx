...
export default function Edit({ auth, group, apartments }) {
    const [search, setSearch] = useState('');

    // Pre-select apartments that currently belong to this group
    const initialSelected = apartments
        .filter(a => a.apartment_group_id === group.id)
        .map(a => a.id);

    const { data, setData, put, processing, errors } = useForm({
        name: group.name,
        apartment_ids: initialSelected,
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('admin.apartment-groups.update', group.id), {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-extrabold text-3xl text-slate-900 tracking-tight leading-none italic uppercase">Edit Group</h2>}
        >
            <Head title="Edit Apartment Group | Apartments24" />
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 md:p-12 space-y-10">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Update Information</p>
                        </div>
                    </div>
                                ...
                                {filteredApartments.length === 0 ? (
                                    ...
                                    <div className="divide-y divide-slate-100 max-h-72 overflow-y-auto">
                                        {filteredApartments.map(apt => {
                                            ...
                                            // In another group = has a group_id, and it's NOT this group, and NOT currently selected
                                            const isInOtherGroup = apt.apartment_group_id
                                                && apt.apartment_group_id !== group.id
                                                && !isSelected;
                                            return (
                                                <label
                                                    key={apt.id}
